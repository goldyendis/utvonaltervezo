...
class MyGeoDataFrame:
    """Class containing as attributes the Osmium OSMObjects, the processing tags and the GeoPanda GeoDataframe.
       Also make access to the method to manipulate the GeoDataframe"""

    def __init__(self, osm_feature, key_tag: str) -> None:
        self.osm_feature = osm_feature
        self.gpdf: geopandas.GeoDataFrame = geopandas.GeoDataFrame(columns=["geometry"])
        self.key_tag = key_tag

    def set_projection(self, epsg: int) -> geopandas.geodataframe:
    ...
    def set_key_columns(self) -> geopandas.GeoDataFrame:
    ...
    def reduce_columns(self, attributes: list) -> geopandas.GeoDataFrame:
    ...
    def rename_column(self, original: str, changed: str) -> geopandas.GeoDataFrame:
        """
    ...
    def extra_columns(self, attributes: List[str]) -> geopandas.GeoDataFrame:
        """
    ...
    def swap_column_values(self, from_column: str, to_column: str, tag_for: List[str] = None) -> geopandas.GeoDataFrame:
        """
    ...
    def get_place_of_worship_value(self) -> geopandas.GeoDataFrame:
        """
        ...
        :return: GeoPandas.GeoDataframe
        """
        if str(self.key_tag) == "place_of_worship":
            ...
            raise Exception("Function should be called on place_of_worship tag")

    def get_man_made_value(self) -> geopandas.GeoDataFrame:
        """
        Process the man_made tag with the tower_type tag, to normalize the return value
        :return: GeoPandas.GeoDataframe
        """
        if str(self.key_tag) == "man_made":
            ...
            raise Exception("Function should be called on man_made_point features")
